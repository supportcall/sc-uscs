...
```
📁 Your Server Root
└── sendmail.php  ← Upload here (accessible at https://sc-uscs.com/sendmail.php)
```
...
1. **User visits sc-uscs.com** → generates custom batch script
2. **User downloads and runs script** → performs Windows cleanup/security tasks
3. **Script collects report data** → computer name, threats found, functions executed
4. **Script sends HTTPS POST** → to `https://sc-uscs.com/sendmail.php` with report data
5. **PHP script receives data** → formats beautiful HTML email
6. **PHP sends via SMTP** → to your configured email addresses
7. **You receive report** → in your inbox within seconds
...
**Generated by SC-USCS v5.84 - Professional Edition**
